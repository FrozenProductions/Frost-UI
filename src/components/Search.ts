...
export class Search {
    ...
    private highlightText(text: string, query: string): string {
        ...
        return `${text.slice(0, index)}<span class="highlight">${text.slice(
            index,
            index + query.length
    ...
    private displayResults(results: SearchResult[], query: string) {
        ...
        const existingEmpty: HTMLDivElement | null = this.resultsContainer.querySelector(".frost-search-empty");
        if (existingEmpty) {
        ...
        for (const result of results) {
            ...
            const typeIndicator: HTMLSpanElement = document.createElement("span");
            ...
            if (this.menus.size > 1) {
                const menuName: HTMLDivElement = document.createElement("div");
                menuName.className = "menu-name";
                menuName.textContent = result.menuName;
                contentWrapper.appendChild(menuName);
            }

            if (result.type === "item") {
                const categoryPath: HTMLSpanElement = document.createElement("span");
                categoryPath.className = "category-path";
                categoryPath.textContent = result.path.join(" > ");
                contentWrapper.appendChild(categoryPath);
            }
